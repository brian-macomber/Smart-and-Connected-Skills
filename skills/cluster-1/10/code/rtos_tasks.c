...
// global variable to control direction of counting (0 = counting up, 1 = counting down)
...
static const uint16_t binaries[] = {
...
static void test_alpha_display()
{
    ...
    while (1)
    {
        if (!direction_flag)
        { //ascending
            displaybuffer[0] = binaries[(int)'U'];
            displaybuffer[1] = binaries[(int)'P'];
            displaybuffer[2] = binaries[(int)'-'];
            displaybuffer[3] = binaries[(int)'-'];
        }
        else
        { //descending
            displaybuffer[0] = binaries[(int)'D'];
            displaybuffer[1] = binaries[(int)'O'];
            displaybuffer[2] = binaries[(int)'W'];
            displaybuffer[3] = binaries[(int)'N'];
        }
...
static void button_task()
{
    ...
    while (1)
    {
        ...
        if (button_flag)
        {
            vTaskDelay(100); //test to make button less sensitive
            //for debuggin
...
void app_main(void)
{
    ...
    xTaskCreate(button_task, "button_task", 4096, NULL, configMAX_PRIORITIES, NULL);
    xTaskCreate(test_alpha_display, "test_alpha_display", 4096, NULL, 5, NULL);
    xTaskCreate(count_in_binary, "count_in_binary", 4096, NULL, configMAX_PRIORITIES - 5, NULL);
}
