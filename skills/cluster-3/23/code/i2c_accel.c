...
void writeRegister(uint8_t reg, uint8_t data)
{
    ...
    //start command
    i2c_master_start(cmd);
    i2c_master_write_byte(cmd, (SLAVE_ADDR << 1) | WRITE_BIT, ACK_CHECK_EN); //slave address followed by write bit
    i2c_master_write_byte(cmd, reg, ACK_CHECK_EN);                           //register pointer sent
    i2c_master_write_byte(cmd, data, ACK_CHECK_DIS);                         //data sent
...
uint8_t readRegister(uint8_t reg)
{
    ...
    //start command
    i2c_master_start(cmd);
    i2c_master_write_byte(cmd, (SLAVE_ADDR << 1) | WRITE_BIT, ACK_CHECK_EN); //slave followed by write bit
    i2c_master_write_byte(cmd, reg, ACK_CHECK_EN);                           //register pointer sent

    //repeated start command
    i2c_master_start(cmd);
    i2c_master_write_byte(cmd, (SLAVE_ADDR << 1) | READ_BIT, ACK_CHECK_EN); //slave followed by read bit
    i2c_master_read_byte(cmd, &value, ACK_CHECK_DIS);                       //place data from register into bus
